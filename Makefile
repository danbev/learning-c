all: kq inherit pthreads var memset memcmp inet_conv fs cp server-socket client-socket signals
...
clean: 
	rm -f kq inherit pthreads var memset memcmp inet_conv fs cp client-socket server-socket
